...
describe('getSpecificRecord', () => {
  describe('getContentHash', () => {
    it('should return null for a non-existent name', async () => {
      const result = await ensInstance.getContentHash('test123123cool.celo')
      expect(result).toBeUndefined()
    })
    it('should return null for a name with no contenthash record', async () => {
      const result = await ensInstance.getContentHash('with-profile.celo')
      expect(result).toBeUndefined()
    })
    it('should return the contenthash for a name with the record set', async () => {
      const result = await ensInstance.getContentHash('with-contenthash.celo')
  ...
  describe('getAddr', () => {
    it('should return the celo addr record if no coinType is provided', async () => {
      const result = await ensInstance.getAddr('with-profile.celo')
    ...
    it('should return the correct address based on a coin ID input as a number', async () => {
      const result = await ensInstance.getAddr('with-profile.celo', 61)
    ...
    it('should return the correct address based on a coin name', async () => {
      const result = await ensInstance.getAddr('with-profile.celo', 'ETC_LEGACY')
      expect((result as any).addr).toBe(
        '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC',
      )
      expect((result as any).coin).toBe('ETC_LEGACY')
    ...
    it('should return null for a non-existent coin', async () => {
      const result = await ensInstance.getAddr('with-profile.celo', 'BNB')
      expect(result).toBeUndefined()
    })
  })
  ...
  describe('getABI', () => {
    it('should return object for type 1 ABI', async () => {
      const result = await ensInstance.getABI('with-type-1-abi.celo')
    ...
    it('should return object for type 2 ABI', async () => {
      const result = await ensInstance.getABI('with-type-2-abi.celo')
      expect(result).toBeTruthy()
      if (result) {
    ...
    it('should return object for type 4 ABI', async () => {
      const result = await ensInstance.getABI('with-type-4-abi.celo')
      expect(result).toBeTruthy()
      if (result) {
    ...
    it('should return unresolved URI for type 8 ABI', async () => {
      const result = await ensInstance.getABI('with-type-8-abi.celo')
      expect(result).toBeTruthy()
      if (result) {
    ...
    it('should return undefined if unsupported contentType', async () => {
      const result = await ensInstance.getABI('with-type-256-abi.celo')
      expect(result).toBeUndefined()
    })
  })
  ...
  describe('_getABI', () => {
    it('should return if resolver address is included', async () => {
      const profile = await ensInstance.getProfile('with-type-1-abi.celo')
      expect(profile?.resolverAddress).toBeTruthy()
      const result = await ensInstance._getABI(
        'with-type-1-abi.celo',
