...
describe('getProfile', () => {
  describe('with an address', () => {
    it('should return a profile object with no other args', async () => {
      ...
      expect(result).toBeDefined()
      if (result) {
        expect((result as any).name).toBe('with-profile.celo')
  ...
  describe('with a name', () => {
    it('should return a profile object with no other args', async () => {
      const result = await ensInstance.getProfile('with-profile.celo')
    ...
    it('should return a profile object for a specified resolver', async () => {
      const tx = await ensInstance.wrapName('test123.celo', {
        wrappedOwner: accounts[1],
        addressOrIndex: 1,
      })
      await tx.wait()
      const result = await ensInstance.getProfile('test123.celo', {
    ...
    it('should return the decoded name for a name with encoded labels', async () => {
      const result = await ensInstance.getProfile(
        '[9dd2c369a187b4e6b9c402f030e50743e619301ea62aa4c0737d4ef7e10a3d49].with-subnames.celo',
      )
      expect(result).toBeDefined()
      expect(result?.decryptedName).toBe('xyz.with-subnames.celo')
    })
    it('should return undefined for an unregistered name', async () => {
      const result = await ensInstance.getProfile('test123123123cool.celo')
      expect(result).toBeUndefined()
    })
  ...
  describe('with an old resolver', () => {
    it('should use fallback mceloods for a name with an older resolver (no multicall)', async () => {
      const result = await ensInstance.getProfile('with-legacy-resolver.celo')
      expect(result).toBeDefined()
      if (result) {
  ...
  describe('with invalid resolver', () => {
    it('should fail gracefully for a name with invalid resolver', async () => {
      const tx = await ensInstance.setResolver('test123.celo', {
        contract: 'registry',
    ...
    it('should fail gracefully for a wrapped name with invalid resolver', async () => {
      const tx = await ensInstance.setResolver('wrapped.celo', {
      ...
      expect(tx).toBeTruthy()
      await tx.wait()
      const result = await ensInstance.getProfile('wrapped.celo')
      expect(result).toBeDefined()
      if (result) {
        expect(result.address).toBeUndefined()
        expect(Object.keys(result.records!).length).toBe(0)
  ...
  describe('errors', () => {
    ...
    it('should throw an celojs error with no data', async () => {
      try {
        await ensInstance.getProfile('with-profile.celo')
    ...
    it('should not throw an celojs error if skipGraph is true', async () => {
      try {
        const result = await ensInstance.getProfile('with-profile.celo', {
          skipGraph: true,
          fallback: {
