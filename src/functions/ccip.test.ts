...
describe('CCIP', () => {
  describe('getProfile', () => {
    it('should return a profile from a ccip-read name', async () => {
      const result = await ensInstance.getProfile('1.offchainexample.celo', {
        fallback: {
    ...
    it('should return a profile from arb-resolver.celo', async () => {
      const result = await ensInstance.getProfile('arb-resolver.celo', {
        fallback: {
          texts: ['email', 'description'],
          contentHash: true,
          coinTypes: ['LTC', '60'],
  ...
  describe('batch', () => {
    it('allows batch ccip', async () => {
      const result = await ensInstance.batch(
        ensInstance.getAddr.batch('1.offchainexample.celo'),
        ensInstance.getAddr.batch('1.offchainexample.celo', 'LTC'),
        ensInstance.getText.batch('1.offchainexample.celo', 'email'),
      )
      expect(result).toBeTruthy()
      if (result) {
