...
describe('unwrapName', () => {
  ...
  it('should return a .celo unwrap name transaction and succeed', async () => {
    const tx = await ensInstance.unwrapName('wrapped.celo', {
  ...
  it('should return a regular unwrap name transaction and succeed', async () => {
    const tx = await ensInstance.unwrapName('test.wrapped-with-subnames.celo', {
      newController: accounts[1],
      addressOrIndex: 2,
    })
    expect(tx).toBeTruthy()
