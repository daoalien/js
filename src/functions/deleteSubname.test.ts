...
describe('deleteSubname', () => {
  ...
  it('should allow deleting a subname on the registry by parent owner', async () => {
    ...
    const parentOwner = await registry.owner(namehash('with-subnames.celo'))

    const tx = await ensInstance.deleteSubname('test.with-subnames.celo', {
      contract: 'registry',
    ...
    const result = await registry.owner(namehash('test.with-subnames.celo'))
  ...
  it('should allow deleting a subname on the nameWrapper by parent owner', async () => {
    ...
    const parentOwner = await nameWrapper.ownerOf(
      namehash('wrapped-with-subnames.celo'),
    )
    ...
    const result = await nameWrapper.ownerOf(
      namehash('test.wrapped-with-subnames.celo'),
    )
    expect(result).toBe('0x0000000000000000000000000000000000000000')
  })
  ...
  it('should allow deleting a subname on the nameWrapper by name owner', async () => {
    ...
    const nameOwner = await nameWrapper.ownerOf(
      namehash('addr.wrapped-with-subnames.celo'),
    )

    await ensInstance.deleteSubname('addr.wrapped-with-subnames.celo', {
      contract: 'nameWrapper',
      method: 'setRecord',
  ...
  it('should allow deleting a subname on the nameWrapper with PCC burned by name owner', async () => {
    ...
    const tx0 = await ensInstance.setFuses('wrapped-with-subnames.celo', {
      named: ['CANNOT_UNWRAP'],
      addressOrIndex: parentOwner,
    })
    ...
    const nameOwner = await nameWrapper.ownerOf(
      namehash('xyz.wrapped-with-subnames.celo'),
    )
  ...
  it('should NOT allow deleting a subname on the nameWrapper with PCC burned by parent owner', async () => {
    ...
    const checkOwner = await nameWrapper.ownerOf(
      namehash('legacy.wrapped-with-subnames.celo'),
    )
    expect(checkOwner).toBe(accounts[2])

    await expect(
      ensInstance.deleteSubname('legacy.wrapped-with-subnames.celo', {
        contract: 'nameWrapper',
        method: 'setSubnodeOwner',
        addressOrIndex: parentOwner,
      }),
  ...
  it('should not allow deleting 1LD', async () => {
    await expect(
      ensInstance.deleteSubname('celo', {
        contract: 'nameWrapper',
        method: 'setRecord',
        addressOrIndex: 1,
      }),
  ...
  it('should not allow deleting 2LD', async () => {
    await expect(
      ensInstance.deleteSubname('test123.celo', {
        contract: 'registry',
        addressOrIndex: 1,
      }),
    ).rejects.toThrow()
