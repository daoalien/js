...
describe('getSubnames', () => {
  it('should get the subnames for a name ordered by createdAt in desc order', async () => {
    ...
    expect(result.subnames[0].name).toEqual('addr.with-subnames.celo')
    expect(result.subnames[1].name).toEqual('xyz.with-subnames.celo')
    expect(result.subnames[2].name).toEqual('legacy.with-subnames.celo')
    expect(result.subnames[3].name).toEqual('test.with-subnames.celo')
    expect(
  ...
  it('should get the subnames for a name ordered by createdAt in asc order', async () => {
    ...
    expect(result.subnames.length).toBe(4)
    expect(result.subnameCount).toBe(4)
    expect(result.subnames[0].name).toEqual('test.with-subnames.celo')
    expect(result.subnames[1].name).toEqual('legacy.with-subnames.celo')
    expect(result.subnames[2].name).toEqual('xyz.with-subnames.celo')
    expect(result.subnames[3].name).toEqual('addr.with-subnames.celo')
    expect(
      result.subnames.every((subname, i, arr) => {
        if (arr[i + 1]) {
          return (subname as any).createdAt <= (arr[i + 1] as any).createdAt
  ...
  it('should get the subnames for a name by labelName in desc order', async () => {
    ...
    expect(result.subnames.length).toBe(4)
    expect(result.subnameCount).toBe(4)
    expect(result.subnames[0].name).toEqual('xyz.with-subnames.celo')
    expect(result.subnames[1].name).toEqual('test.with-subnames.celo')
    expect(result.subnames[2].name).toEqual('legacy.with-subnames.celo')
    expect(result.subnames[3].name).toEqual('addr.with-subnames.celo')
    testProperties(
      result.subnames[0],
  ...
  it('should get the subnames for a name by labelName in asc order', async () => {
    ...
    expect(result.subnames[0].name).toEqual('addr.with-subnames.celo')
    expect(result.subnames[1].name).toEqual('legacy.with-subnames.celo')
    expect(result.subnames[2].name).toEqual('test.with-subnames.celo')
    expect(result.subnames[3].name).toEqual('xyz.with-subnames.celo')
    testProperties(
      result.subnames[0],
      'id',
      'labelName',
  ...
  describe('wrapped subnames', () => {
    it('should return fuses', async () => {
      const result = await ensInstance.getSubnames({
        name: 'wrapped-with-subnames.celo',
        pageSize: 10,
        orderBy: 'createdAt',
        orderDirection: 'desc',
      })
    ...
    it('should return expiry as undefined if 0', async () => {
      const result = await ensInstance.getSubnames({
        name: 'wrapped-with-expiring-subnames.celo',
        pageSize: 10,
        orderBy: 'createdAt',
        orderDirection: 'desc',
      })
  ...
  describe('with pagination', () => {
    ...
    describe('with pagination', () => {
      it('should get paginated subnames for a name ordered by createdAt in desc order', async () => {
        ...
        expect(result2.subnames[0].name).toEqual('legacy.with-subnames.celo')
        expect(result2.subnames[1].name).toEqual('test.with-subnames.celo')
        expect(
      ...
      it('should get paginated subnames for a name ordered by createdAt in asc order', async () => {
        ...
        expect(result2.subnames.length).toBe(2)
        expect(result2.subnameCount).toBe(4)
        expect(result2.subnames[0].name).toEqual('xyz.with-subnames.celo')
        expect(result2.subnames[1].name).toEqual('addr.with-subnames.celo')
        expect(
          result2.subnames.every((subname, i, arr) => {
            if (arr[i + 1]) {
              return (subname as any).createdAt <= (arr[i + 1] as any).createdAt
      ...
      it('should get paginated subnames for a name by labelName in asc order', async () => {
        ...
        expect(result2.subnames.length).toBe(2)
        expect(result2.subnameCount).toBe(4)
        expect(result2.subnames[0].name).toEqual('test.with-subnames.celo')
        expect(result2.subnames[1].name).toEqual('xyz.with-subnames.celo')
    ...
    describe('With search query', () => {
      ...
      it('should get the searched subnames for a name ordered by createdAt in asc order', async () => {
        ...
        expect(result.subnames.length).toBe(2)
        expect(result.subnameCount).toBe(4)
        expect(result.subnames[0].name).toEqual('legacy.with-subnames.celo')
        expect(result.subnames[1].name).toEqual('addr.with-subnames.celo')
        expect(
          result.subnames.every((subname, i, arr) => {
            if (arr[i + 1]) {
              return (subname as any).createdAt <= (arr[i + 1] as any).createdAt
    ...
    describe('with search query and pagination', () => {
      ...
      it('should get paginated subnames for a name ordered by createdAt in asc order', async () => {
        ...
        expect(result2.subnames.length).toBe(1)
        expect(result2.subnameCount).toBe(4)
        expect(result2.subnames[0].name).toEqual('addr.with-subnames.celo')
  ...
  describe('wrapped', () => {
    it('should get the subnames for a name ordered by createdAt in desc order', async () => {
      ...
      expect(result.subnames.length).toBe(4)
      expect(result.subnameCount).toBe(4)
      expect(result.subnames[0].name).toEqual('addr.wrapped-with-subnames.celo')
      expect(result.subnames[1].name).toEqual('xyz.wrapped-with-subnames.celo')
      expect(result.subnames[2].name).toEqual(
        'legacy.wrapped-with-subnames.celo',
      )
      expect(result.subnames[3].name).toEqual('test.wrapped-with-subnames.celo')
      expect(
        result.subnames.every((subname, i, arr) => {
          if (arr[i + 1]) {
            return (subname as any).createdAt >= (arr[i + 1] as any).createdAt
    ...
    it('should get the subnames for a name ordered by createdAt in asc order', async () => {
      ...
      expect(result.subnames.length).toBe(4)
      expect(result.subnameCount).toBe(4)
      expect(result.subnames[0].name).toEqual('test.wrapped-with-subnames.celo')
      expect(result.subnames[1].name).toEqual(
        'legacy.wrapped-with-subnames.celo',
      )
      expect(result.subnames[2].name).toEqual('xyz.wrapped-with-subnames.celo')
      expect(result.subnames[3].name).toEqual('addr.wrapped-with-subnames.celo')
      expect(
        result.subnames.every((subname, i, arr) => {
          if (arr[i + 1]) {
            return (subname as any).createdAt <= (arr[i + 1] as any).createdAt
    ...
    it('should get the subnames for a name by labelName in desc order', async () => {
      ...
      expect(result.subnames.length).toBe(4)
      expect(result.subnameCount).toBe(4)
      expect(result.subnames[0].name).toEqual('xyz.wrapped-with-subnames.celo')
      expect(result.subnames[1].name).toEqual('test.wrapped-with-subnames.celo')
      expect(result.subnames[2].name).toEqual(
        'legacy.wrapped-with-subnames.celo',
      )
      expect(result.subnames[3].name).toEqual('addr.wrapped-with-subnames.celo')
    ...
    it('should get the subnames for a name by labelName in asc order', async () => {
      ...
      expect(result.subnames[2].name).toEqual('test.wrapped-with-subnames.celo')
      expect(result.subnames[3].name).toEqual('xyz.wrapped-with-subnames.celo')
      testProperties(
        result.subnames[0],
        'id',
        'labelName',
    ...
    describe('with pagination', () => {
      it('should get paginated subnames for a name ordered by createdAt in desc order', async () => {
        ...
        expect(result.subnames[0].name).toEqual(
          'addr.wrapped-with-subnames.celo',
        )
        expect(result.subnames[1].name).toEqual('xyz.wrapped-with-subnames.celo')
        expect(
          result.subnames.every((subname, i, arr) => {
      ...
      it('should get paginated subnames for a name ordered by createdAt in asc order', async () => {
        ...
        expect(result2.subnameCount).toBe(4)
        expect(result2.subnames[0].name).toEqual(
          'xyz.wrapped-with-subnames.celo',
        )
        expect(result2.subnames[1].name).toEqual(
          'addr.wrapped-with-subnames.celo',
        )
  ...
  describe(' errors', () => {
    ...
    it('should throw an error with no data if celojs-debug is set to CELOJSUknownError', async () => {
      try {
        const result = await ensInstance.getSubnames({
          name: 'with-subnames.celo',
          pageSize: 10,
        ...
        const error = e as CELOJSError<Result>
