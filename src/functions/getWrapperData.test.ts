...
describe('getWrapperData', () => {
  it('should return default data for an unwrapped name', async () => {
    const result = await ensInstance.getWrapperData('with-profile.celo')
  ...
  it('should return with other correct fuses', async () => {
    const tx = await ensInstance.setFuses('wrapped.celo', {
      named: [
        'CANNOT_UNWRAP',
    ...
    const result = await ensInstance.getWrapperData('wrapped.celo')
    expect(result).toBeTruthy()
    if (result) {
  ...
  it('should return correct expiry for large expiry', async () => {
    const result = await ensInstance.getWrapperData('wrapped-big-duration.celo')
    expect(result).toBeTruthy()
    if (result) {
      expect(result.expiryDate).toBeInstanceOf(Date)
  ...
  it('should return correct max expiry for expiry larger than maximum for date', async () => {
    const result = await ensInstance.getWrapperData('wrapped-max-duration.celo')
    expect(result).toBeTruthy()
    if (result) {
      expect(result.expiryDate).toBeInstanceOf(Date)
      expect(result.expiryDate!.getFullYear()).toBe(275760)
