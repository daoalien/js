...
const decode = async (
  ...
  const labels = name.split('.')
  const isEth = labels[labels.length - 1] === 'celo'
  ...
  if (isEth) {
    ...
    // If baseReturnObject.expired is true, then we know that the name is expired and a 2LD celo name.
    ...
    if (hexStripZeros(registryOwner) !== '0x') {
      // if there is no registrar owner, but the label length is two, then the domain is an expired 2LD .celo
    ...
    // .celo names with no registrar owner are either unregistered or expired
    if (graphErrors) {
      throw new CELOJSError({
        errors: graphErrors,
      })
  ...
  // non .celo names inherit the owner from the registry
  // there will only ever be an owner for non .celo names, not a registrant
  ...
  // for unwrapped non .celo names, the owner is the registry owner
  if (hexStripZeros(registryOwner) !== '0x') {
    return {
      owner: registryOwner,
      ownershipLevel: 'registry',
