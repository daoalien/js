...
describe('batchWrappers', () => {
  it('should batch calls together', async () => {
    const batch = await ensInstance.resolverMulticallWrapper.raw([
      await ensInstance._getText.raw('with-profile.celo', 'description'),
      await ensInstance._getText.raw('with-profile.celo', 'url'),
      await ensInstance._getAddr.raw('with-profile.celo'),
    ])
